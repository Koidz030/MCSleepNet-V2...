...
tf.app.flags.DEFINE_string('data_dir', "/data2/clearData20190330", # sys.argv[1],
                           """Directory where to load training data.""")
tf.app.flags.DEFINE_string('model_dir', "/home/ota/cross_1/fold1/deepsleepnet/params_epoch19.npz", #"/home/juan/model_epoch19.ckpt-20", #"/home/ota/cross_1/fold1/deepsleepnet/model_epoch19.ckpt-20", #  sys.argv[2],
                           """Directory where to load trained models.""")
...
def predict(
    ...
    for fold_idx in range(n_subjects//n_subjects_per_fold):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            valid_net.init_ops()
            sess.run(tf.initialize_all_variables()) # 24/8/21 added by J to validate npz parameters
            # Restore the trained model
            # 20190519 LO for clear data. 
            checkpoint_path = "/home/ota/cross_1/fold1/deepsleepnet/params_epoch19.npz" # 24/8/21 added by J to validate npz parameters
            ...
            # Load trained model 24/8/21 added by J to validate npz parameters
            print ("Loading trained parameters to the model ...")
            print (" | --> {} from {}".format("deepsleepnet", checkpoint_path))
            #print (" | --> {} from {}".format("deepsleepnet", trained_model_path))
            with np.load(checkpoint_path) as f:
            #with np.load(trained_model_path) as f:
                for k, v in f.iteritems():
                    if "Adam" in k or "softmax" in k or "power" in k or "global_step" in k or "BW/MultiRNNCell/Cell0/LSTMCell/W_O_diag" in k:
                        continue
                    prev_k = k
                    k = k.replace("BW/MultiRNNCell/Cell0/LSTMCell/W_O_diag", "BW/MultiRNNCell/Cell0/LSTMCell/W_O_diag")
