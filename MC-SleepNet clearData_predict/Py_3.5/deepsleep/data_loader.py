...
if clearDataFlag:
    ...
    upperLimit = 17280#17275#17000 # epoch (This value must be rational to 25, and less than the length of input eeg, emg epoch number.)
else:
...
class NonSeqDataLoader(object):
    ...
    def _load_csv_file(self, csv_file):
        ...
        if clearDataFlag:
            ...
            labels[labels=="NR"] = 1
            labels[labels=="R"] = 2
        return list(eeg[:upperLimit]), list(emg[:upperLimit]), list(labels[:upperLimit])
    ...
    def load_subject_data7(data_dir, mouse):
        ...
        def load_sub_file(csv_file,data_dir):
            ...
            if upperLimit < epoch_num:
                ...
                emg = emg[0:upperLimit, :]
            if clearDataFlag:
                labels = pd.read_csv("%s/training_20s/madeByLO/%s_training.csv"%(data_dir,csv_file), header = 0, index_col=0).values.flatten()
                labels[labels=="W"] = 0
                labels[labels=="NR"] = 1
                labels[labels=="R"] = 2
