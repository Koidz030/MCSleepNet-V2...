...
def custom_bidirectional_rnn(cell_fw, cell_bw, inputs,
    ...
    #weight for LSTM results - 06/16 J added 
    weight = 4

    flat_outputs = tuple(array_ops.concat(1, [fw, weight*bw])
...
def custom_run_epoch(
    ...
    for eeg, emg in iterate_batch_seq_minibatches(eeg=each_x_1,
        ...
        #all_logits = np.concatenate([all_logits, logits], axis=0) # 2021/6/4 LO removed.

        _, y_pred, logits, fw_state, bw_state = sess.run(
            [train_op, network.pred_op, network.logits, network.fw_final_state, network.bw_final_state],
            feed_dict=feed_dict
        ) # 2021/6/4 LO added logits output for each 25 epoch sequence.
        ######### 2021/6/4 LO added to save logit output. ############
        if len(all_logits)==0:
            all_logits = logits
        else:
    ...
    if extra != 0:
        ...
        _, y_pred = sess.run(
            [train_op, network.pred_op],
            feed_dict = feed_dict
        )
        
        y_pred = y_pred[-extra:]
        each_y_pred.extend(y_pred)
        all_logits = np.concatenate([all_logits, logits[-extra:]], axis=0) # 2021/6/4 LO added.

    # _, y_pred, logits, fw_state, bw_state = sess.run([train_op, network.pred_op, network.logits, fw_final_state, network.bw_final_state], feed_dict=feed_dict) # 2021/6/4 LO removed.
    ...
    y_pred[y_pred==1] = 49
    y_pred[y_pred==2] = 82
    pd.DataFrame(y_pred,index=range(1,len(y_pred)+1)).to_csv("predict/output_subject.csv",header=False,index=True)
    
    ######## 2021/6/4 LO added to save logit output. ##############
    pd.DataFrame(all_logits, index=range(1,all_logits.shape[0]+1)).to_csv("predict/output_subject_logits.csv", header=False, index=True) # W, NR, R logit output for each epochs.
    ######## 2021/6/4 LO added to save logit output. ##############
