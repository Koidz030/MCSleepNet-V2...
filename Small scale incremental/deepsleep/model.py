...
class DeepFeatureNet(object):
    ...
    def build_model(self, input_eeg_var, input_emg_var):
        ...
        # Convolution
        # network = self._conv1d_layer(input_var=input_var, filter_size=125, n_filters=64, stride=15, wd=1e-3)
        ...
        if self.use_dropout:
            ...
            if self.is_train:
                ...
                #network = tf.nn.dropout(network, rate=0.0, name=name)
            self.activations.append((name, network))
        self.layer_idx += 1
        # Convolution
        ...
        ######### EEG CNNs with large filter size at the first layer #########
        # Convolution
        # network = self._conv1d_layer(input_var=input_var, filter_size=1024, n_filters=64, stride=128)
        network = self._conv1d_layer(input_var=input_eeg_var, filter_size=500, n_filters=64, stride=50)
        # Max pooling
        name = "l{}_pool".format(self.layer_idx)
        network = max_pool_1d(name=name, input_var=network, pool_size=4, stride=4)
        self.activations.append((name, network))
        ...
        output_conns.append(network)
        """
        ######### EMG CNNs with small filter size at the first layer #########
        # Convolution
        # network = self._conv1d_layer(input_var=input_var, filter_size=125, n_filters=64, stride=15, wd=1e-3)
        network = self._conv1d_layer(input_var=input_emg_var, filter_size=125, n_filters=64, stride=15, wd=1e-3)
        # Max pooling
        name = "l{}_pool".format(self.layer_idx)
        network = max_pool_1d(name=name, input_var=network, pool_size=8, stride=8)
        self.activations.append((name, network))
...
class DeepSleepNet(DeepFeatureNet):
    ...
    def build_model(self, input_eeg_var, input_emg_var):
        ...
        with tf.variable_scope(name) as scope:
            ...
            #output_tmp = fc(name="fc", input_var=network, n_hiddens=512, bias=None, wd=0)# 2020/9/29 LEO OTA revised. ## 1024, bias=None, wd=0)
        ...
        #hidden_size = 256# 2020/9/29 LEO OTA revised. ## 512   # will output 1024 (512 forward, 512 backward)
        with tf.variable_scope(name) as scope:
    ...
    def init_ops(self):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            print(" ")
            print("sequence_loss_by_example shape")
            print(loss)
            print(" ")
