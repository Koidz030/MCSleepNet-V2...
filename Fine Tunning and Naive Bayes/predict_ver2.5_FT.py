...
class CustomDeepSleepNet(DeepSleepNet):
    ...
    def build_model(self, input_eeg_var, input_emg_var):
        ...
        hidden_size = 256   # will output 512 (256 forward, 256 backward)
        with tf.variable_scope(name) as scope:
...
def custom_run_epoch(
    ...
    y_pred_consol=[define_stage_forEachSleepEpoch(prob1, prob2) for prob1, prob2 in zip(each_y_pred_prob_LSTM, each_y_pred_prob_NB)] #J modified 09/04/21
